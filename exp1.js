...
function initialize(){
  ...
  */
  ...
  sim.C.getConstraint("ChemotaxisConstraint", 1).deltaH = dH
    // End Manuel BugFix
  */
...
function drawBelow() {
  ...
  this.Cim.drawPixelSet(this.finish, "00FF00");
}

// thanks chatgpt :))
function mixRGBA(c1, c2) {
  ...
  const r = (r1 * a1 + r2 * a2) / total;
  const g = (g1 * a1 + g2 * a2) / total;
  const b = (b1 * a1 + b2 * a2) / total;

  // Keep it consistent with the same weighting idea
  const a = 1 - (1 - a1) * (1 - a2)
